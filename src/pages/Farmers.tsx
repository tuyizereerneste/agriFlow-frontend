...
const Farmers: React.FC = () => {
  ...
  const fetchFarmers = async () => {
    ...
    try {
      const response = await axios.get<{ farmers: Farmer[] }>("https://agriflow-backend-cw6m.onrender.com/api/search", {
  ...
  const exportFarmers = async () => {
    try {
      const response = await axios.get("https://agriflow-backend-cw6m.onrender.com/api/farmers/export-excel", {
        headers: { Authorization: `Bearer ${token}` },
        responseType: "blob",
      });
