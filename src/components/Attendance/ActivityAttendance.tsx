...
export const ActivityAttendance: React.FC<ActivityAttendanceProps> = ({ projectId, projectTitle, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchProjectPractices = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get<{ data: Practice[] }>(`https://agriflow-backend-cw6m.onrender.com/api/project/project-practices/${projectId}`, {
  ...
  useEffect(() => {
    if (selectedPractice) {
      const fetchPracticeActivities = async () => {
        ...
        setError(null);
        try {
          const response = await axios.get<{ data: Activity[] }>(`https://agriflow-backend-cw6m.onrender.com/api/project/practice-activities/${selectedPractice}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
...
const handleCapture = (capturedFile: File) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('https://agriflow-backend-cw6m.onrender.com/api/project/attendance', formData, {
        headers: {
